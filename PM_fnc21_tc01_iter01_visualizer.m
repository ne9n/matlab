...
H2_subplots = 7;
    ...
    sp2(1) = subplot(H2_subplots,1,1);  % <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        hold on
%        yyaxis left; 
            plot(SimResults.tout,test_signal_trimmed,'-k','LineWidth',1,'DisplayName','shunt')
            %plot(time(xlim_Xamps_start:xlim_Xamps_stop),test_signal(xlim_Xamps_start:xlim_Xamps_stop),'-k','LineWidth',1,'DisplayName','shunt')
...
sp2(6) = subplot(H2_subplots,1,6); % <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
 ...
 sp(7) = subplot(H2_subplots,1,7);
        hold on
        yyaxis left; 
            plot(SimResults.tout,-1*SimResults.X_peakamps_at_fault,'-k','LineWidth',1,'DisplayName','peak fault A'); 
            ylabel('Amperage (A)'); ylim([-1700 100])
            ax = gca; ax.YColor = 'k';
        yyaxis right; 
            plot(SimResults.tout,-1*SimResults.X_peakamps_at_protect,'-.r','LineWidth',1,'DisplayName','peak protect A'); 
            ylabel('Amperage (A)'); ylim([-1700 100])
            ax = gca; ax.YColor = 'r';
        ...
        grid minor
        xlabel('seconds'); %xlim([xlim_Xload_start xlim_Xload_stop])
        title('Peak Current at Excessive Amps Fault and Protect')
        legend('show','Location','west')
        box on
