...
%       
    ...
    sp_Xregen(6) = subplot(H_Xregen_subplots,1,6); % <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     sp_Xregen(7) = subplot(H_Xregen_subplots,1,7);  % <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        annotation('textbox','String',str_A,'BackgroundColor','w','FontSize',8,'Position',[0.78 0.8 0.2 0.2],'FitBoxToText','on')
        
        tr_title = "EXCESSIVE REGEN CURRENT MONITORING, DIAGNOSTICS & METRICS - ";
        str_sgtitle = str_title + str_app + newline + str_TestConditions;
