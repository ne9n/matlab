% PURPOSE: this signal building script makes it simple to construct
% one-testsignal-does-full-regression test
...
test_dur = 12*60; % units = minutes then multiply by 60 sec/min to get test_dur in units of seconds
...
%% EXERCISE EXCESSIVE DSICHARGE CURRENT MONITORING, REPORTING & PROTECTION
% Design Requirements:
%   1) 
%   2) 
%
% Test Plan:  hit the algorithm with the following variety of discharge pules to exercise its component parts:
% 1) 106 amp discharge pulse 950 ms duration (just 1 Amp above the 1Dlookup table threshold and shy of the 1 sec duration to trigger a fault level)
% 2) 106 amp discharge pulse 2900 ms duration (just 1 Amp above the 1Dlookup table threshold and > 1 sec to trigger fault level but shy of the protect duration)
% 3) 

    % Pulse #7: Test 0 sec Xamps threshold event, count, timer
        ...
        %==================================
            ...
            test_signal(X7_start:X7_stop,1) = 106; 
            test_signal(R7_start:R7_stop,1) = 20;

    % Pulse #8: Test > 1sec Xamps threshold event, count, timer
        ...
        %==================================
            ...
            test_signal(X8_start:X8_stop,1) = 106; 
            test_signal(R8_start:R8_stop,1) = 20;

    % Pulse #9: Test > 1sec Xamps threshold event, count, timer
    ...
    % Pulse #10: Test > 1sec Xamps threshold event, count, timer
    ...
    % Pulse #11: Test > 1sec Xamps threshold event, count, timer
    ...
    % Pulse #12: Test > 6.5sec Xamps threshold event, count, timer
        Xpulse12_start = R11_stop + 1;
        %==================================
        Xload_pulse12_duration_ms = 6500; % exceed Xload threshold for 1 sec < dur < 3 sec start 0 sec and 1 sec event counters and timer
        Rest_pulse12_duration_ms = 500; % drop below Xload threshold
        %==================================
            ...
            X12_stop = X12_start + Xpulse12;
            R12_start = X12_stop + 1;
            R12_stop = R12_start + Xrest12;

            test_signal(X12_start:X12_stop,1) = 108; 
            test_signal(R12_start:R12_stop,1) = 20;
            
    % Pulse #13: Test > 10sec Xamps threshold event, count, timer
        Xpulse13_start = R12_stop + 1;
        %==================================
        Xload_pulse13_duration_ms = 10000; % exceed Xload threshold for 1 sec < dur < 3 sec start 0 sec and 1 sec event counters and timer
        Rest_pulse13_duration_ms = 500; % drop below Xload threshold
        %==================================
            Xpulse13 = uint16(Xload_pulse13_duration_ms/rate_ms);  % uints = number of execution loops
            Xrest13 = uint16(Rest_pulse13_duration_ms/rate_ms);  % uints = number of execution loops
            X13_start = Xpulse13_start;
            X13_stop = X13_start + Xpulse13;
            R13_start = X13_stop + 1;
            R13_stop = R13_start + Xrest13;

            test_signal(X13_start:X13_stop,1) = 108; 
            test_signal(R13_start:R13_stop,1) = 20;

    % Pulse #14: Test Xamps duration lookup table using a -440 Amps for 1.2 sec (goes into table breakpoint #6)
        Xpulse14_start = R13_stop + 1;
        %==================================
        Xload_pulse14_duration_ms = 1200; % exceed Xload threshold for 1 sec < dur < 3 sec start 0 sec and 1 sec event counters and timer
        Rest_pulse14_duration_ms = 500; % drop below Xload threshold
        %==================================
            Xpulse14 = uint16(Xload_pulse14_duration_ms/rate_ms);  % uints = number of execution loops
            Xrest14 = uint16(Rest_pulse14_duration_ms/rate_ms);  % uints = number of execution loops
            X14_start = Xpulse14_start;
            X14_stop = X14_start + Xpulse14;
            R14_start = X14_stop + 1;
            R14_stop = R14_start + Xrest14;

            test_signal(X14_start:X14_stop,1) = 440; 
            test_signal(R14_start:R14_stop,1) = 20;

    % Pulse #15: Test Xamps duration lookup table using a -1500 Amps for 1 sec (goes into table breakpoint #6)
        Xpulse15_start = R14_stop + 1;
        %==================================
        Xload_pulse15_duration_ms = 300; % exceed Xload threshold for 1 sec < dur < 3 sec start 0 sec and 1 sec event counters and timer
        Rest_pulse15_duration_ms = 500; % drop below Xload threshold
        %==================================
            Xpulse15 = uint16(Xload_pulse15_duration_ms/rate_ms);  % uints = number of execution loops
            Xrest15 = uint16(Rest_pulse15_duration_ms/rate_ms);  % uints = number of execution loops
            X15_start = Xpulse15_start;
            X15_stop = X15_start + Xpulse15;
            R15_start = X15_stop + 1;
            R15_stop = R15_start + Xrest15;

            test_signal(X15_start:X15_stop,1) = 1500; 
            test_signal(R15_start:R15_stop,1) = 20;

testsignal_shunt = [time,test_signal];
time_trimmed = time(46000:52500);
time_trimmed_rezeroed = time_trimmed - time_trimmed(1);
test_signal_trimmed = test_signal(46000:52500);
testsignal_shunt_Xamps = [time_trimmed_rezeroed,test_signal_trimmed];
test_samples = length(testsignal_shunt_Xamps);
...
% optional write testsignal with time to a spreadsheet for use by the HiL test system
    writematrix(testsignal_shunt,'PM_Xamps_testsignal.xlsx')